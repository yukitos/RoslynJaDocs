...
**missing token**(訳注：不明なトークン)を挿入します。
...
シンボルには参照しているその他のシンボルなど、
コンパイラがソースコードやメタデータから得た追加情報が含まれます。
(訳注：原文は
"Symbols also contain additional information that the compiler
determined from the source or metadata, such as other referenced symbols."
「シンボルにはそれを参照する他のシンボルなど」が正しい？)
