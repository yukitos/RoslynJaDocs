...
ツリー変換機能(tree transformations)を作成することになります！

## ツリーの作成および「変更」

### ファクトリーメソッドを使用してノードを作成する

**シンタックスノード** を作成するには **SyntaxFactory** クラスの
ファクトリーメソッドを使用します。
...
2. `SyntaxFactory` クラスのファクトリーメソッドを修飾せずに
...
5. `ReplaceNode` 拡張メソッドを呼び出して新しいツリーを作成して、
   既存のインポートノードを変更後の `newUsing` ノードに置き換え、
   既存の変数 `root` にこの新しいツリーを格納するような以下のコードを追記します：
